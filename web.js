document.addEventListener('DOMContentLoaded', () => {
    ...
    const TECH_TAG_COLORS = {
        'embedded systems': '255, 132, 76',
        'c++': '181, 137, 255',
        'sensor fusion': '255, 180, 48',
        'autonomous robotics': '78, 190, 255',
        'autonomous driving': '78, 190, 255',
        'python': '78, 190, 255',
        'pytorch': '244, 114, 182',
        'opencv': '0, 217, 255',
        'pid': '255, 180, 48',
        'control systems': '255, 132, 76',
        'classic vision': '0, 217, 255'
    };

    const TECH_TAG_FALLBACKS = [
        '255, 132, 76',
        '181, 137, 255',
        '255, 180, 48',
        '78, 190, 255',
    ...
    function formatTechTag(tag) {
        ...
        return normalizeTechTag(cleanTag) === 'pid' ? 'PID' : cleanTag;
    }

    function fallbackTechTagColor(key) {
        let hash = 0;
        for (let i = 0; i < key.length; i++) {
            hash = ((hash << 5) - hash) + key.charCodeAt(i);
            hash |= 0;
        }
        return TECH_TAG_FALLBACKS[Math.abs(hash) % TECH_TAG_FALLBACKS.length];
    }

    function techTagColor(tag) {
        const key = normalizeTechTag(tag);
        return TECH_TAG_COLORS[key] || fallbackTechTagColor(key);
    }

    function renderTechTags() {
        document.querySelectorAll('.tech-stack').forEach(stack => {
            const sourceTags = stack.dataset.tags
                ? stack.dataset.tags.split(',')
                : [...stack.querySelectorAll('span')].map(tag => tag.textContent);
            const tags = sourceTags.map(tag => tag.trim()).filter(Boolean);
            if (tags.length === 0) return;

            stack.replaceChildren(...tags.map(tag => {
                const chip = document.createElement('span');
                chip.textContent = formatTechTag(tag);
                chip.style.setProperty('--tag-rgb', techTagColor(tag));
                return chip;
            }));
        });
    }
